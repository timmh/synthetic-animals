...
with inpkg():
    ...
    from .utils import encode_inst_id, encode_class_id
...
class ExrDict(dict):
    ...
    def get_class(self):
        rgb = self.get_rgb()
        classes = encode_class_id.rgb_to_id(rgb)

        # if world.use_nodes is False, Blender will set background as a gray (0.05087609, 0.05087609, 0.05087609)
        gray_background_mask = (rgb[..., 0] != 0) & (rgb[..., 0] != 1)
        classes[gray_background_mask] = -1
        return classes


class ImageWithAnnotation(dict):
    def __init__(self, image=None, exr=None, exr_class=None):
        self["image"] = image
        self["inst"] = exr.get_inst()
        self["class"] = exr_class.get_class()
        self["depth"] = exr.get_depth()
        self["_raw_exr"] = exr
        self["_raw_exr_class"] = exr_class
    ...
    def vis(self):
        ...
        class_vis = greyToRgb(histEqualize(self["class"]))
        vis = (
            np.concatenate([inst_vis, class_vis, image[..., :3] / 255.0, depth_vis], 1)
            * 255
        ).astype(np.uint8)
        return vis
    ...
    def save(self, dataset_dir="dataset", fname="0"):
        ...
        if self.get("class") is not None:
            class_dir = pathjoin(dataset_dir, "class_map")
            os.makedirs(class_dir, exist_ok=True)
            class_path = pathjoin(class_dir, fname + ".png")
            cv2.imwrite(class_path, self["class"].clip(0).astype(np.uint16))
