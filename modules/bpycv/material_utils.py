...
class set_class_material(StatuRecover):
    def __init__(self):
        StatuRecover.__init__(self)
        ...
        for obj_idx, obj in enumerate(objs):
            if "class_id" in obj:
                ...
                class_id = -1  # -1 as default class_id
            color = tuple(encode_class_id.id_to_rgb(class_id)) + (1,)

            material_name = "auto.class_material." + obj.name
            material = bpy.data.materials.new(material_name)
            material["is_auto"] = True
            material.use_nodes = True
            material.node_tree.nodes.clear()
