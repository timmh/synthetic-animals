...
def randomize_particles():
    plane = [obj for obj in bpy.context.scene.objects if obj.name.startswith("Ground")][0]
...
def import_animal(animal_data, SCENE_VARIATION):
    ...
    # TODO: remove workaround for proper deer display
...
def main():
    ...
    SCENE_VARIANT = os.getenv("SCENE_VARIANT", "day").lower()
    ...
    for i in tqdm(range(1, N_SAMPLES + 1)):
        ...
        for j in range(random.randint(MIN_N_ANIMALS, MAX_N_ANIMALS)):
            ...
            animal = import_animal(animal_data, SCENE_VARIANT)
        ...
        cv2.imwrite(
            ...
            cv2.cvtColor(rgb, cv2.COLOR_RGB2BGR) if SCENE_VARIANT != 'night' else cv2.cvtColor(rgb, cv2.COLOR_RGB2GRAY),
        )
